...
|---|---|---|---|
| **BUN (Urea)** | One marker aliases both *BUN* (≈7–20 mg/dL) and *Urea* (≈15–40 mg/dL) — different conventions ~2.14× apart — but grades both against the BUN band with `criticalHigh: 47`. A normal "Urea ~50" trips a **false `critical`** (cliff is checked before `labRef`); manual-entry "Urea 21–40" reads `concern`. | **false alarm** (incl. false critical) | Split into two markers (`bun` 7–20 / `urea` 15–40), each with its own scale, band, and cliff. |
...
| **SHBG** | Upper bound `50 nmol/L` is below most lab ceilings (≈57–76); SHBG rises with age, so healthy older men can read "high." Only bites without a `labRef`; SHBG is interpretive, low stakes. | false alarm | Raise max to ≥57 (or age-band). |
| **HbA1c** | Thresholds are correct (5.7 / 6.5). But HbA1c is unreliable in Indian populations (iron-deficiency anaemia → spuriously high; thalassaemia/G6PD → either way). | risk of false assurance/alarm via assay, not via our numbers | Surface a caveat: confirm a borderline/raised HbA1c with fasting glucose, interpret alongside a CBC. (The action-tick label already carries the anaemia note.) |
| **eGFR** | Values are fine, but a 30–59 result (CKD G3, below the KDIGO 60 cutoff) carries no flag of its own — it sits outside the 90–150 band yet only prose conveys the significance. | mild false assurance | Add a non-critical "below normal (<60)" tier so the clinically meaningful cutoff is visible, reserving red for ≤30. |
